...
public:
    TransitionOOP(
        std::vector<PlacePoint> input_places, std::vector<int> input_weights,
        std::vector<PlacePoint> output_places, std::vector<int> output_weights,
    ...
    void run() {
        if (this->m_func == nullptr) { // 没有挂载函数
            ...
            for (int i = 0; i < m_input_places.size(); ++i) {
                for (int j = 0; j < m_input_weights[i]; ++j)
                    m_input_tokens.push_back(std::move(m_input_places[i]->output_tokens())); // 取出的tokens放到Transition的缓存中，准备输给挂载的函数
            }
            ...
            for (int i = 0; i < m_output_places.size(); ++i) {
                for (int j = 0; j < m_output_weights[i]; ++j)
                    m_output_places[i]->input_tokens(std::move(m_output_tokens[i]));
            }
        }
    }
};
